...
<div class="bg-white p-6 rounded-lg shadow-md">
  ...
  <form action="{{ route('meterans.update', $meteran->id) }}" method="POST">
    ...
    <div class="grid md:grid-cols-2 gap-4 mb-4">
      ...
      <div>
        ...
        <select name="periode_id" class="w-full border-gray-300 rounded-md">
          ...
          @foreach ($periodes as $periode)
            <option value="{{ $periode->id }}" {{ $periode->id == $meteran->periode_id ? 'selected' : '' }}>
      ...
      <div>
        ...
        <select name="tarif" id="tarif" class="w-full border-gray-300 rounded-md">
          ...
          @foreach ($tarifs as $tarif)
            <option value="{{ $tarif->harga_per_m3 }}" {{ $tarif->harga_per_m3 == $meteran->tarif ? 'selected' : '' }}>
      ...
      <div>
        ...
        <input type="number" step="0.01" name="jumlah_meteran" id="jumlah_meteran" class="w-full border-gray-300 rounded-md" value="{{ $meteran->jumlah_meteran }}" required>
      </div>

      {{-- TOTAL TAGIHAN (OTOMATIS) --}}
      <div>
        <label for="total_tagihan_display" class="block font-medium">Total Tagihan</label>
        <input type="text" id="total_tagihan_display" class="w-full border-gray-300 rounded-md bg-gray-100" readonly value="Rp {{ number_format($meteran->total_tagihan, 0, ',', '.') }}">
        <input type="hidden" name="total_tagihan" id="total_tagihan" value="{{ $meteran->total_tagihan }}">
      </div>
      ...
      <div>
        ...
        <select name="metode_bayar" class="w-full border-gray-300 rounded-md">
          ...
          <option value="CASH" {{ $meteran->metode_bayar == 'CASH' ? 'selected' : '' }}>CASH</option>
        </select>
      </div>
  ...
  value="{{ $meteran->tanggal_bayar ? \Carbon\Carbon::parse($meteran->tanggal_bayar)->format('Y-m-d') : '' }}">
      </div>
      ...
      <div>
        ...
        <select name="petugas_id" class="w-full border-gray-300 rounded-md">
          ...
          @foreach ($petugas as $user)
            <option value="{{ $user->id }}" {{ $user->id == $meteran->petugas_id ? 'selected' : '' }}>
    ...
    <button type="submit" class="bg-blue-700 text-white px-4 py-2 rounded-md hover:bg-blue-800">Update</button>
  </form>
</div>
@endsection
